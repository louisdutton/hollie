...
Rooms author grass independently from their floor material through the optional `layers.grass` array. Each cell is an unsigned density from 0 (bare ground) to 255 (all 36 deterministic blade placements). Missing layers contain no grass, and intermediate densities retain a stable subset across reloads.
