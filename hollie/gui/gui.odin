...
init :: proc() {
	...
	ctx.camera = rl.Camera2D{zoom = ctx.ui_scale}
}
...
checkbox :: proc(rect: Rectangle, label_text: string, checked: ^bool) -> bool {
	...
	if checked^ {
		...
		margin: f32 = 3
		rl.DrawRectangleRec(
			Rectangle{rect.x + margin, rect.y + margin, rect.width - 2 * margin, rect.height - 2 * margin},
			check_color,
		)
	}
	...
	// Draw label
	if len(label_text) > 0 {
		rl.DrawText(cstring(raw_data(label_text)), i32(rect.x + rect.width + 5), i32(rect.y + 2), 16, rl.BLACK)
	}
...
text_input :: proc(rect: Rectangle, text: ^string, max_length: int) -> bool {
	mouse_pos := get_mouse_pos()
	hovered := rl.CheckCollisionPointRec(mouse_pos, rect)
	changed := false

	// Simple text input (placeholder implementation)
	// In a full implementation, you'd handle cursor, selection, etc.

	// Draw background
	color := hovered ? rl.WHITE : Color{250, 250, 250, 255}
	rl.DrawRectangleRec(rect, color)
	rl.DrawRectangleLinesEx(rect, 1, rl.BLACK)

	// Draw text
	if len(text^) > 0 {
		rl.DrawText(cstring(raw_data(text^)), i32(rect.x + 5), i32(rect.y + 5), 16, rl.BLACK)
	}
...
/// Draw a progress bar
progress_bar :: proc(rect: Rectangle, progress: f32, bg_color := rl.LIGHTGRAY, fg_color := rl.BLUE) {
	// Clamp progress to 0-1 range
	clamped_progress := max(0, min(progress, 1))
	...
	if clamped_progress > 0 {
		fill_rect := Rectangle{
			rect.x,
			rect.y,
			rect.width * clamped_progress,
			rect.height,
		}
		rl.DrawRectangleRec(fill_rect, fg_color)
	}
}