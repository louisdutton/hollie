...
rendering_camera :: proc() -> rl.Camera3D {
...
rendering_draw_character :: proc(
	...
	if clip_index >= 0 {
		...
		if blend < 1 && previous_clip_index >= 0 {
			...
			rl.UpdateModelAnimation(model_assets.character, clip, clip_frame)
		}
	}
	...
	for &entity in entities {
		...
		case Pressure_Plate:
			...
			if clip_index >= 0 {
				...
				rl.UpdateModelAnimation(model_assets.pressure_pad, clip, clip_frame)
			}
			rl.DrawModelEx(
...
rendering_draw_particles :: proc() {
	for &particle in particle_system.particles {
		...
		rl.DrawSphere(geometry_position(particle.position, 3), particle.size * 0.35, color)
	}
}
