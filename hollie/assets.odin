...
MODEL_CHARACTER_CLIP_NAMES :: [Animation_State]string {
	.Idle  = "idle",
	.Run   = "walk",
...
MODEL_CHARACTER_PLAYBACK :: [Animation_State]Animation_Playback {
	.Idle  = .Loop,
	.Run   = .Loop,
	.Jump  = .Loop,
	.Death = .Once_Hold,
...
Model_Assets :: struct {
	...
	character_animation_indices:    [Animation_State]int,
...
model_assets_init :: proc() {
	...
	for state_index := 0; state_index < len(clip_names); state_index += 1 {
		animation_state := Animation_State(state_index)
